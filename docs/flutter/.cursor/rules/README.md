...
These project rules are derived from the shared `/docs` source of truth and the technology-specific `/flutter/docs` documentation.

## Precedence
1. Explicit product/business requirements in `/docs`.
2. Technology-specific documentation in `/flutter/docs`.
3. Applicable `.mdc` rules in `/flutter/.cursor/rules`.
...
## Scope
These rules guide development only for the `flutter` codebase. Shared product rules remain in `/docs`.
